...
static ge::graphStatus TilingFunc(gert::TilingContext *context) {
    ...
    if (board_cast) {
        ...
        int64_t ub_per_vector = block_per_vector * length_per_block *
                                ((data_type == ge::DataType::DT_FLOAT)
                                     ? (4 * 3 + 1) // fp32
                                     : ((data_type == ge::DataType::DT_FLOAT16)
                                            ? (2 * 3 + 1)            // fp16
                                            : (2 * 3 + 4 * 2 + 1))); // bf16
        ...
        int64_t ub_per_block =
            length_per_block * ((data_type == ge::DataType::DT_FLOAT)
                                    ? (4 * 3 + 1) // fp32
                                    : ((data_type == ge::DataType::DT_FLOAT16)
                                           ? (2 * 3 + 1)            // fp16
                                           : (2 * 3 + 4 * 2 + 1))); // bf16
